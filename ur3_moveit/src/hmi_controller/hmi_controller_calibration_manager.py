...
from utils import util_ros_msgs as util_ros
...
class CalibrationManager(): # TODO separate into Calibration and Transform managers
    ...
    def publish_tf(self, quaternion_hmi):
        if self.debug: 
            ...
            self.tf_pub.sendTransform(tfs_north_direction) # this TF is correct only when calibrated - it should be aligned with absolute X axis (world X)
    ...
    def __get_frame_calibration(self):
        # here we use the "pure" orientation, without the correction
