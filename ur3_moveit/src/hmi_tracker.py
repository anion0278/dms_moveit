...
class HmiTracker:
    ...
    def on_data(self, depth_msg, img_msg):
        ...
        self.__process_hand_data(right_hand, config.hmi_right, self.right_pc_pub, depth_msg)
        self.__process_hand_data(left_hand, config.hmi_left, self.left_pc_pub, depth_msg)

        if debug:
            cycle_time = (rospy.get_time() - start) / 1000.0
            print ("Tracker - time spent: %s ms" % cycle_time)

    def __process_hand_data(self, hand_data, hmi_name, pc_pub, depth_msg):
        if (hand_data is not None):
            self.publish_hand(depth_msg, depth_msg.header, hand_data, hmi_name,pc_pub)
        else:
    ...
    def publish_hand(self, depth_msg,header,hand_data,hmi_name,pc_pub):
        cloud_center, radius = self.pc_proc.get_center_and_publish(pc_pub, hand_data, depth_msg, header)
